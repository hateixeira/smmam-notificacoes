...
// O ROBÔ DOS CORREIOS (LAZY CRON AUTOMATION & FORÇADO) COM FEEDBACK
window.verificarRotinaCorreios = async function(forcar = false) {
    ...
    try {
        ...
        if (dadosAuto[campoTurno] !== dataHoje || forcar) {
            ...
            if(forcar && btnForcar) { btnForcar.innerText = "⏳ Consultando..."; btnForcar.disabled = true; }
...
window.registrarUsuario = async function() { 
    ...
    // VERIFICAÇÃO DE DOMÍNIO E LISTA VIP
    let isVip = false;
    try {
...
window.buscarConsultaLivre = async function(tipoBusca) {
    ...
    if (tipoBusca === 'lote') {
        ...
        // Remove palavras comuns para não buscar "RUA" e trazer o banco inteiro
        ...
        // Busca pela primeira palavra relevante dentro da Array invisível gerada pelo Delta Sync
        ...
        if(!snap.empty) {
            ...
            snap.forEach(docSnap => {
                ...
                if(tipoBusca === 'endereco') {
                    ...
                    // O banco trouxe pela rua. Agora o seu PC (RAM) filtra o número exato de graça!
        ...
        try {
            ...
            dadosNovosArray.forEach(im => { 
                if(im.chaveinscricao) {
                    // O SEGREDO DA BUSCA DE ENDEREÇO: Cria a Array Invisível
...
window.salvarDocumento = async function(event, tipoDoc) {
    ...
    if(tipoDoc === 'notificacao') {
        ...
        document.getElementById('numNotif').value = numeroOriginal; // Devolve pra tela
...
// DEBOUNCE NA BUSCA (Impede o PC de travar enquanto você digita)
let searchTimeout;
window.onBuscaKeyUp = function() {
    clearTimeout(searchTimeout);
    searchTimeout = setTimeout(() => {
        window.renderizarPainel();
    }, 400); // Aguarda 0.4s após você parar de digitar para filtrar
}
