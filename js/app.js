...
// O ROBÔ DOS CORREIOS (COM PROXY E FREIO DE RATE LIMIT)
window.verificarRotinaCorreios = async function(forcar = false) {
    ...
    try {
        ...
        if (dadosAuto[campoTurno] !== dataHoje || forcar) {
            ...
            if(forcar && btnForcar) { btnForcar.innerText = "⏳ Consultando (Pode levar alguns segundos)..."; btnForcar.disabled = true; }

            if(!forcar) await setDoc(autoRef, { [campoTurno]: dataHoje }, { merge: true });
            console.log(`[LazyCron] Iniciando rastreamento Motor Duplo. Forçado: ${forcar}`);
            ...
            for (let document of pendingSnaps.docs) {
                ...
                // 1. Tentativa via Proxy na BrasilAPI
                try {
                    const urlBrasil = `https://corsproxy.io/?${encodeURIComponent('https://brasilapi.com.br/api/correios/v1/' + d.codigoAR)}`;
                    const res = await fetch(urlBrasil);
                    if (res.ok) {
                ...
                // 2. Tentativa via Proxy na Linketrack
                if (!desc) {
                    try {
                        const urlLinke = `https://corsproxy.io/?${encodeURIComponent('https://api.linketrack.com/track/json?user=teste&token=1abcd00b2731640e886fb41a8a9671ad1434c599dbaa0a0de9a5aa619f29a83f&codigo=' + d.codigoAR)}`;
                        const res2 = await fetch(urlLinke);
                        if (res2.ok) {
...
// AÇÃO MANUAL DA TABELA (API) COM PROXY
window.buscarStatusCorreios = async function(codigoAR, spanId, docId) {
    ...
    try {
        ...
        try {
            const urlBrasil = `https://corsproxy.io/?${encodeURIComponent('https://brasilapi.com.br/api/correios/v1/' + codigoAR)}`;
            const res = await fetch(urlBrasil);
            if (res.status === 404) {
        ...
        if (!desc) {
            try {
                const urlLinke = `https://corsproxy.io/?${encodeURIComponent('https://api.linketrack.com/track/json?user=teste&token=1abcd00b2731640e886fb41a8a9671ad1434c599dbaa0a0de9a5aa619f29a83f&codigo=' + codigoAR)}`;
                const res2 = await fetch(urlLinke);
                if (res2.ok) {
                    const data2 = await res2.json();
                    if (data2.eventos && data2.eventos.length > 0) desc = data2.eventos[0].status.toLowerCase();
...
// ATUALIZADO: DELTA SYNC IPTU (Compara em RAM + Cria Array de Buscas)
...
// SALVAR DOCUMENTO COM REGRA DE DUPLICIDADE (/ANO)
...
window.renderizarPainel = function() {
    ...
    filtrados.forEach(item => {
        ...
        if(item.codigoAR) { 
            ...
            let tituloTooltip = item.statusCorreiosTexto ? `Status Completo: ${item.statusCorreiosTexto}` : `Aguardando atualização dos Correios.`;

            // TOOLTIP INSERIDA NO "TITLE"
            statusHtml += `
            <div title="${tituloTooltip}" style="${corFisica} padding:6px; border-radius:4px; border:1px solid; text-align:center; min-width: 140px; box-shadow: 0 1px 2px rgba(0,0,0,0.05); cursor:help;">
...
window.imprimirRegistro = function(id) {
    ...
    if(document.getElementById('pNome')) document.getElementById('pNome').innerText = (item.nome || '').toUpperCase(); 
    if(document.getElementById('pDoc')) document.getElementById('pDoc').innerText = item.doc; 
...
window.exportarExcel = function() {
    if(window.itensFiltradosAtual.length === 0) return alert("Vazio."); let c = "\uFEFFNº Reg;Tipo;Ouvidoria;Data;Nome;CPF/CNPJ;Lote Irregular;Bairro;Cidade;Prazo;Codigo AR;Status AR;Fiscal\n";
    window.itensFiltradosAtual.forEach(i => { c += `${i.numNotif || ''};${(i.tipoDocumento||'').toUpperCase()};${i.procOuvidoria || ''};${i.dataNotif ? i.dataNotif.split('-').reverse().join('/') : ''};${(i.nome||'').toUpperCase().replace(/;/g,',')};${i.doc||''};${(i.loteEndereco||'').replace(/;/g,',')};${i.bairro||''};${i.cidade||''};${i.dataPrazo ? i.dataPrazo.split('-').reverse().join('/') : ''};${i.codigoAR||''};${i.statusRetornoAR||''};${i.fiscal||''}\n`; });
